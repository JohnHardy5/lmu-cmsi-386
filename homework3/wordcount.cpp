  ...
  for (unsigned int i = 0; i < input.length(); i++) {
    if (isspace(input[i])) {
      ...
      for (unsigned int j = i + 1; j < input.length(); j++) {
        ...
        //std::cout << "Position of counter: " << repeatFinderPosition << " Position of j: " << j << '\n';
        if (isspace(input[j])) {
          if (input.substr(repeatFinderPosition, j - repeatFinderPosition + 1) == currentWord) {
            //std::cout << "Found a match: " << input.substr(repeatFinderPosition, j - repeatFinderPosition + 1) << "and " << currentWord << '\n';
            numberOfWords++;
